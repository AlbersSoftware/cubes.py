...
def generate_polycubes(n, use_cache=True):
    ...
    list(np.array): Returns a list of all polycubes of size n as numpy byte arrays
      if __name__ == "__main__":
        N = 7  # You can set your desired value of N here
        use_cache = True  # Set to True or False as needed
        polycubes = generate_polycubes(N, use_cache)
    """
...
def render_shapes(shapes):
    ...
    # Iterate over the polycubes and render them as solid cubes
        ...
        for y in range(voxel_array.shape[1]):
            for z in range(voxel_array.shape[2]):
                if voxel_array[x, y, z] > 0:
                    ax.bar3d(x, y, z, 1, 1, 1, shade=True, color='#FFD65DC0')

    ax.set_xlim([0, voxel_dim])
    ax.set_ylim([0, voxel_dim])
    ax.set_zlim([0, dim])
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    plt.show()
cubes_data = np.load("Cubes/cubes_4.npy", allow_pickle=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cache', action='store_true', default=True, help='Enable caching (default: True)')
    ...
    # Call the rendering function to visualize the polycubes
    render_shapes(all_cubes)
